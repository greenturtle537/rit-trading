...
document.getElementById('adForm').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (!token) {
            messageDiv.innerHTML = '<p style="color: red;">You must be logged in to post an ad. <a href="/trading/login">Login here</a></p>';
            return;
        }
        ...
        if (response.ok) {
            ...
            setTimeout(() => {
                window.location.href = `/trading/category?category=${category}`;
            }, 2000);
        } else {
            // Failed to post ad
